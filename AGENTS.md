...
For A/B comparisons, save a known anchor, save each changed tree as a named
variant, then compare stored runs offline with `LOAD` and `BASELINE`. Refresh
...
- With `CORE` unset, Linux runs acquire a lock for the selected performance
  core. An explicit `CORE=<n>` bypasses automatic selection and locking.
...
covered by a fixture test, and remains outside `BENCH=all`.
