...
anchor_manifest= candidate_manifest=

while (($#)); do
	case "$1" in
	--criterion-root | --snapshot-root | --arch | --comparison | --pair | --target | --anchor-run | --candidate-run | --anchor-commit | --candidate-commit | --anchor-manifest | --candidate-manifest)
...
for name in criterion_root snapshot_root arch comparison pair target anchor_run candidate_run anchor_commit candidate_commit anchor_manifest candidate_manifest; do
...
python3 - "$anchor_manifest" "$candidate_manifest" "$anchor_commit" "$candidate_commit" "$arch" "$validation" "$REPO_ROOT" <<'PY'
import hashlib
import json
import os
import subprocess
...
anchor_path, candidate_path, anchor_commit, candidate_commit, arch, output, repo = sys.argv[1:]
...
def validate(manifest, path, supplied_commit):
...
validate(anchor, anchor_path, anchor_commit)
validate(candidate, candidate_path, candidate_commit)
json.dump({"control": anchor["control"]}, open(output, "w", encoding="utf-8"), indent=2, sort_keys=True)
...
elastic_cache_gate | funnel_cache_gate)
	comparison_command=(cargo bench --locked --bench "$target" -- "${criterion_args[@]}")
	(cd "$REPO_ROOT" && CRITERION_HOME="$criterion_root" "${comparison_command[@]}") >"$comparison_stdout" 2>"$comparison_stderr"
	python3 - "$comparison_commands" "${comparison_command[@]}" <<'PY'
import json,sys
json.dump([sys.argv[2:]], open(sys.argv[1], "w"), indent=2)
PY
	;;
scaled_insert)
	comparison_command=(cargo bench --locked --bench scaled_insert -- "${criterion_args[@]}")
	(cd "$REPO_ROOT" && CRITERION_HOME="$criterion_root" "${comparison_command[@]}") >"$comparison_stdout" 2>"$comparison_stderr"
	python3 - "$comparison_commands" "${comparison_command[@]}" <<'PY'
import json,sys
json.dump([sys.argv[2:]], open(sys.argv[1], "w"), indent=2)
...
all)
	...
	for bench in speedup mean_latency; do
		command=(cargo bench --locked --bench "$bench" -- "${criterion_args[@]}")
		(cd "$REPO_ROOT" && CRITERION_HOME="$criterion_root" "${command[@]}") >>"$comparison_stdout" 2>>"$comparison_stderr"
		commands+=("$bench")
	done
	python3 - "$comparison_commands" "$candidate_run" "$anchor_run" <<'PY'
import json,sys
candidate, anchor = sys.argv[2:]
json.dump([
    ["cargo", "bench", "--locked", "--bench", bench, "--", "--load-baseline", candidate, "--baseline", anchor]
    for bench in ("speedup", "mean_latency")
...
python3 - "$temporary/pair-manifest.json" "$temporary/build/anchor-manifest.json" "$temporary/build/candidate-manifest.json" "$validation" "$comparison_commands" "$original_command" "$start_ns" "$end_ns" "$start_iso" "$end_iso" "$target" "$anchor_run" "$candidate_run" <<'PY'
import json,platform,sys
output, anchor_path, candidate_path, validation_path, commands_path, snapshot_command, start_ns, end_ns, start_iso, end_iso, target, anchor_run, candidate_run = sys.argv[1:]
