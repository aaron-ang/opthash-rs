...
def test_with_options_defaults(map_cls):
    m = map_cls.with_options()
    assert len(m) == 0
    m[1] = 2
    m["a"] = 1
    assert m[1] == 2
...
def test_with_options_custom_kwargs(map_cls):
    m = map_cls.with_options(capacity=128, reserve_exponent=3)
    for i in range(50):
        m[i] = i * 2
        m[f"k{i}"] = i
    for i in range(50):
